...
class Users_model extends MY_Model
{
    ...
    {
        $this->db->set('spin_remaining', 'golden_wheel_star' . $spin_count, false);
        $this->db->set('updated_date', date('Y-m-d H:i:s'));
        $this->db->where('id', $user_id);
        $this->db->update('tbl_users');
 ...
 FROM `tbl_users` WHERE `referred_by`=tbl_referral_bonus_log.referred_user_id) as refer_count', false)
    ...
    public function GetUsers($postData=null)
    {
        ...
        | <a href="'.base_url('backend/user/delete/' . $record->id).'" class="btn btn-danger"
            ...
            $data[] = array(
              ...
              "on_table"=>($record->table_id > 0 || $record->rummy_table_id > 0 ||$record->ander_bahar_room_id > 0||$record->dragon_tiger_room_id > 0||$record->seven_up_room_id > 0||$record->jhandi_munda_id > 0) ? 'Yes' : 'No',
              "status"=>$status,
   ...
   FROM tbl_purchase where user_id="'.$id.'"
    ...
    UNION
    SELECT 
    "Admin Add/Deduct" as game,id as reff_id,id as bet_id,user_id,"" as winning_amount,added_date,"0" as amount,coin as user_amount, 0 as is_game
    FROM tbl_wallet_log where user_id="'.$id.'"
    UNION
    SELECT 
